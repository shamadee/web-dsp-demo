...
//start filters below
  ...
  void convFilter (float* data, int width, int height, float* kern, int kWidth, int kHeight, float divisor, double bias, int count) {
    float r, g, b;
    ...
    for (int i = 0; i < count; ++i) {
      for (int y = kCenterY; y < height - kCenterY; ++y) {
        for (int x = kCenterX; x < width - kCenterX; ++x) {
          ...
          pix = (width * y + x) * 4;
          data[pix + 0] = r / divisor;
          data[pix + 1] = g / divisor;
          data[pix + 2] = b / divisor;
        }
      }
    }
  }
